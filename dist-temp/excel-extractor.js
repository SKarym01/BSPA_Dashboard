...
class ExcelExtractor {
    ...
    parseMatrixFromSheet() {
        ...
        for (let r = headerRow + 1; r < this.rawData.length; r++) {
            ...
            if (!paramName)
                continue;
            if (this.isCurveSectionTitle(paramName))
                continue;
    ...
    extractCurveBlockSeries(titleRow, config, cleanColEntries) {
        ...
        for (const variantEntry of cleanColEntries) {
            const points = this.buildCurvePointsForVariant(xRows, yRows, variantEntry.col, config.xValuesByVariant !== false, config.yValuesByVariant !== false);
            if (points.length === 0)
                continue;
    ...
    buildCurvePointsForVariant(xRows, yRows, variantCol, xValuesByVariant, yValuesByVariant) {
        const points = [];
    ...
    isCurveSectionTitle(value) {
        const normalized = (0, text_norm_1.normalizeText)(value, 'strong');
        return this.curveBlockConfigs.some(config => (0, text_norm_1.normalizeText)(config.title, 'strong') === normalized)
            || this.curveBlockConfigs.some(config => (0, text_norm_1.normalizeText)(config.groupName, 'strong') === normalized);
    }
    ...
    readCurveRows(startRow, cleanColEntries, rowMarkerCol, rowMarkerPattern, dataValueCol) {
        const rows = [];
