...
export class NewBspaComponent implements OnInit {
  ...
  exportResultsToExcel() {
    if (!this.can('export_results')) return;
    const results: any = this.mockResults ?? {};

    const workbook = utils.book_new();
    const generatedAt = new Date();
    const safeEpc = String(this.epcNumber || this.dataService.projectData.epcNumber || 'BSPA').replace(/[^a-zA-Z0-9_-]+/g, '_');
    const timestamp = `${generatedAt.getFullYear()}${String(generatedAt.getMonth() + 1).padStart(2, '0')}${String(generatedAt.getDate()).padStart(2, '0')}_${String(generatedAt.getHours()).padStart(2, '0')}${String(generatedAt.getMinutes()).padStart(2, '0')}`;

    const overviewRows: Array<{ Field: string; Value: string }> = [
      { Field: 'EPC Number', Value: this.epcNumber || this.dataService.projectData.epcNumber || '-' },
      { Field: 'Overall Status', Value: String(results.overallStatus ?? '-') },
      { Field: 'Generated At', Value: generatedAt.toISOString() },
      { Field: 'Generated By', Value: this.roleService.selectedRoleLabel },
    ];
    for (const info of results.trackingInfo ?? []) {
      overviewRows.push({
        Field: String(info?.label ?? ''),
        Value: String(info?.value ?? '')
      });
    }
    utils.book_append_sheet(workbook, utils.json_to_sheet(overviewRows), 'Overview');

    const summaryRows = (results.summaryCards ?? []).map((card: any) => ({
      Metric: String(card?.label ?? ''),
    ...
    for (const category of results.categories ?? []) {
      for (const detail of category?.details ?? []) {
        detailRows.push({
          ...
          CategoryStatus: String(category?.status ?? ''),
          Parameter: String(detail?.name ?? ''),
          SimulatedValue: String(detail?.value ?? ''),
          ReferenceGoal: String(detail?.reference ?? ''),
          Unit: String(detail?.unit ?? ''),
          Assessment: String(detail?.assessment ?? '')
        });
      }
    }
    utils.book_append_sheet(workbook, utils.json_to_sheet(detailRows), 'Detailed Results');

    writeFile(workbook, `bspa_simulation_results_${safeEpc}_${timestamp}.xlsx`);
  }
